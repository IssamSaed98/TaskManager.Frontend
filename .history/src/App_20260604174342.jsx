...
function App() {
  ...
  // استدعاء الـ Hook هنا يضمن تفعيله بمجرد تحميل التطبيق
  const { isSubscribed, isSupported, subscribe, unsubscribe } = usePushNotifications()

  useEffect(() => {
    if (token && userRole !== 'Admin') loadTasks()
  }, [token, userRole])

  // تفعيل الاشتراك تلقائياً للموظف بمجرد تسجيل الدخول إذا كان المتصفح يدعم ذلك ولم يكن مشتركاً بعد
  useEffect(() => {
    if (token && isSupported && !isSubscribed) {
      subscribe(); 
    }
  ...
  // إذا كان مدير، نمرر له أزرار الإشعارات أيضاً داخل لوحته إذا رغبت، أو نفتح له لوحته مباشرة
  if (userRole === 'Admin') {
  ...
  return (
    <div className="flex flex-col min-h-screen" style={{ background: '#0d1117', direction: isRTL ? 'rtl' : 'ltr' }}>
      ...
      {/* بقية كود الـ HTML كما هو دون تغيير لضمان عمل التصميم بشكل سليم */}
      {showForm && (
