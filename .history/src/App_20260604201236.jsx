...
const completedCount = tasks.filter(t => t.isCompleted).length
  ...
  style={{ background: '#0d1117', direction: isRTL ? 'rtl' : 'ltr' }}>
     ...
     <span className="font-bold text-white text-sm">✦ TaskFlow</span>
      ...
      <LanguageSwitcher />
  {isSupported && (
  <button onClick={isSubscribed ? unsubscribe : subscribe} className="text-xs px-2 py-1 rounded-lg transition-all"
...
border: `0.5px solid ${isSubscribed ? 'rgba(34,197,94,0.2)' : 'rgba(14,165,233,0.2)'}`,}}>
  ...
  <button key={btn.key} onClick={() => setActiveFilter(btn.key)}
    ...
    {btn.label}
      ...
      {error && ( <div className="mx-4 mt-3 rounded-xl p-3 text-xs text-center" style={{ background: 'rgba(239,68,68,0.1)', color: '#f87171' }}>
