...
function AdminDashboard({ onLogout }) {
  ...
  const navItems = [
    { key: 'home',      icon: '🏠', label: t('admin_dashboard'), labelKey: 'admin_dashboard' },
    { key: 'employees', icon: '👥', label: t('employees'),       labelKey: 'employees' },
    { key: 'tasks',     icon: '📋', label: t('all_tasks'),       labelKey: 'all_tasks' },
    { key: 'reports',   icon: '📊', label: t('reports'),         labelKey: 'reports' },
  ]
  ...
  const StatsGrid = () => (
    <div className="grid grid-cols-2 gap-3 mb-4">
      {[
        ...
        { label: t('active_tasks'), value: completedTasks, color: '#4ade80', bg: 'rgba(34,197,94,0.08)', border: 'rgba(34,197,94,0.2)', icon: '✅' },
    ...
    <div>
      <div className="flex items-center justify-between mb-3">
        <span className="text-sm font-semibold text-white">
          {selectedUser ? `مهام ${selectedUser.username}` : 'اختر موظفاً'}
        </span>
        {selectedUser && (
      ...
      {showForm && (
        <div className="rounded-2xl p-4 mb-4 flex flex-col gap-3" style={{ background: '#0a0f1a', border: '0.5px solid #1e2d40' }}>
          <input value={title} onChange={e => setTitle(e.target.value)}  placeholder={t('task_title')}
      ...
      {!selectedUser ? (
        <div className="flex flex-col items-center justify-center py-16" style={{ color: '#3a5070' }}>
          ...
          <p className="text-xs">اختر موظفاً من قائمة الموظفين</p>
        </div>
      ) : userTasks.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-16" style={{ color: '#3a5070' }}>
          <div className="text-4xl mb-3">📭</div>
  ...
  const Reports = () => (
    <div>
      <div className="text-sm font-semibold text-white mb-4">تقرير الفريق</div>
      <div className="rounded-2xl p-5 mb-3" style={{ background: '#0a0f1a', border: '0.5px solid #1e2d40' }}>
        <div className="text-xs mb-3" style={{ color: '#3a5070' }}>نسبة إنجاز الفريق</div>
        ...
        return (
          <div key={user.id} className="rounded-2xl p-4 mb-3" style={{ background: '#0a0f1a', border: '0.5px solid #1e2d40' }}>
            ...
            <div className="flex gap-3">
              <span className="text-xs" style={{ color: '#4ade80' }}>✅ {user.completedTasks} منجزة</span>
              <span className="text-xs" style={{ color: '#fbbf24' }}>⚡ {user.activeTasks} نشطة</span>
              <span className="text-xs" style={{ color: '#3a5070' }}>📋 {user.totalTasks} إجمالي</span>
            </div>
          </div>
        )
      })}
  ...
  {t(navItems.find(n => n.key === activePage)?.labelKey || 'admin_dashboard')}
</span>
<LanguageSwitcher />
      <div className="flex items-center gap-3 px-4 py-3 sticky top-0 z-50" style={{ background: '#0a0f1a', borderBottom: '1px solid #1e2d40' }}>
        ...
        <span className="text-white font-semibold text-sm flex-1">
          {navItems.find(n => n.key === activePage)?.label || 'لوحة المدير'}
        </span>
        {orgInfo && (
