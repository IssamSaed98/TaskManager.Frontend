...
function AdminDashboard({ onLogout }) {
  ...
  const priorityConfig = {
    High:   { tag: 'bg-red-900/30 text-red-300',    label: <>🔴 <span className="text-red-500">Hoch</span></> },
    Medium: { tag: 'bg-yellow-900/30 text-yellow-300', label: <>🟡 <span className="text-red-500">Mittel</span></> },
    Low:    { tag: 'bg-green-900/30 text-green-300',  label: <>🟢 <span className="text-red-500">Niedrig</span></> },
  }
      ...
      {[
        ...
        <div key={i} className="rounded-2xl p-4 flex items-center gap-3"
          ...
          <div>
            ...
            <div className="text-xs mt-0.5" style={{ color: '#7090b0' }}>{s.label}</div>
          </div>
        </div>
      ))}
    </div>
