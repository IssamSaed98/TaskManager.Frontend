...
function Login({ onLogin, goToRegister }) {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      setError('إيميل أو كلمة مرور غلط')
    } finally {
      setLoading(false)
    }
  }
        ...
        <div className="rounded-2xl overflow-hidden border"
          ...
          <div className="p-9">
            ...
            <div className="relative flex rounded-xl p-1 mb-6"
              ...
              {/* Liquid fill */}
              <div className="absolute top-1 bottom-1 rounded-lg transition-all duration-500"
                style={{
                  width: 'calc(50% - 4px)',
              ...
              <button onClick={() => setRole('employee')}
                className="flex-1 py-2 text-xs font-medium relative z-10 transition-all duration-400 flex items-center justify-center gap-1"
                style={{ color: !isAdmin ? '#e2f0ff' : 'rgba(14,165,233,0.25)' }}>
                👤 موظف
              </button>
            ...
            <div className="mb-5">
              <h2 className="text-xl font-bold mb-1" style={{ color: '#e2f0ff' }}>
                {isAdmin ? 'لوحة تحكم المدير 👑' : 'مرحباً بعودتك 👋'}
              </h2>
              <p className="text-xs" style={{ color: 'rgba(14,165,233,0.35)' }}>
                {isAdmin ? 'ادخل بصلاحيات المدير وتحكم بالفريق' : 'سجّل دخولك للوصول إلى مهامك'}
              </p>
            </div>
