...
function Register({ onRegister, goToLogin }) {
    ...
    >
      ...
      <div className="w-full max-w-sm px-4 pt-14 pb-8 z-10">
        ...
        >
          ...
          {/* Role buttons — بسيطة بدون انيميشن   rgba(2,11,26,0.4)*/}
          <div className="flex gap-2 mb-4">
  <button
    onClick={() => setRole('employee')}
    {/* تم إضافة flex items-center justify-center gap-2 هنا */}
    className="flex-1 py-2 rounded-xl text-xs font-medium flex items-center justify-center gap-2"
    style={{
      background: !isAdmin ? 'rgba(34,184,255,0.15)' : 'rgba(2,11,26,0.4)', // قمت بتعديل اللون 'blue' ليتناسق مع الزر الآخر
      border: `1px solid ${!isAdmin ? 'rgba(34,184,255,0.4)' : 'rgba(34,184,255,0.08)'}`,
      color: !isAdmin ? '#22B8FF' : 'rgba(148,180,220,0.4)',
    }}
  >
    <img src={mitAr} style={myStyleSecond} alt="employee icon" /> 
    <span>{t('employee')}</span>
  </button>

  <button
    onClick={() => setRole('admin')}
    {/* تم إضافة flex items-center justify-center gap-2 هنا أيضاً */}
    className="flex-1 py-2 rounded-xl text-xs font-medium flex items-center justify-center gap-2"
    style={{
  ...
  >
    <img src={cheff} style={myStyleSecond} alt="admin icon" />
    <span>{t('admin')}</span>
  </button>
</div>
